...
import { RefreshCw, Plane, AlertTriangle, Shield, Target, Radio, Satellite, Camera, FileText } from 'lucide-react';
...
const AircraftMapVisualization: React.FC = () => {
  ...
  const exportAsImage = async () => {
    ...
    try {
      ...
      link.download = `aircraft-map-${new Date().toISOString().slice(0,19).replace(/:/g,'-')}.png`;
    ...
    try {
      ...
      if (printWindow) {
        printWindow.document.write(`
          <html><head><title>Aircraft Map Export</title>
          <style>@media print { body { margin: 0; } img { width: 100%; } }</style>
          </head><body>
          <h2 style="font-family:monospace;margin:8px 0">AIRCRAFT SURVEILLANCE MAP — ${new Date().toLocaleString()}</h2>
          <p style="font-family:monospace;font-size:12px;color:#666">${filteredFlights.length} aircraft displayed | Filter: ${filter} | Source: ${sourceFilter}</p>
          <img src="${imgData}" style="width:100%;border:1px solid #ccc"/>
          <p style="font-family:monospace;font-size:10px;color:#999;margin-top:8px">Generated by Surveillance Command Center</p>
          </body></html>
        `);
  ...
  return (
    <Card ref={cardRef} className="col-span-full border-destructive/30 bg-card/50 backdrop-blur">
