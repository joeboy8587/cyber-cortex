  ...
  const fetchCoverageData = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < 90; i++) {
        ...
        // (otherwise stress without flight correlation could just be missing data)
        const phantomStressReliable = hasAdequateCoverage;
    ...
    >
          ...
          <div className="grid grid-cols-7 gap-1">
            {coverageData.slice(0, 63).map((day) => (
              <div
                ...
                title={`${day.date}: ${day.flightCount} flights, ${day.biometricCount} biometrics`}
              >
      ...
      <div className="mt-4 flex items-center gap-4 text-xs text-muted-foreground">
        ...
        <div className="flex items-center gap-1">
          ...
          <span>Phantom stress reliable</span>
        </div>
        <div className="flex items-center gap-1">
          <XCircle className="w-3 h-3 text-red-400" />
          <span>Phantom stress unreliable</span>
        </div>
      </div>
    </CyberPanel>
  );
