...
import { resolveFaaIdentities, decideIdentity } from "../_shared/faaIdentity.ts";
...
// tagged tier2_shell or medical_air.
...
function classifyAircraft(registration: string, callsign: string, altitude: number, speed: number = 0): ClassificationResult {
  ...
  if (isScheduledAirlineCallsign(call) && altitude >= 10000) {
    return {
      ...
      entity: 'Scheduled Airline', entityType: 'commercial',
  ...
  // Medical air patterns — anchored, not substring (AMX665 is Aeromexico, not medical).
