...
export async function handleAction3(action: string, body: Record<string, any>, sql: SQL): Promise<unknown> {
  switch (action) {
    ...
    case 'getLegalAnalysisStats': {
      const safe = <T,>(p: Promise<T>): Promise<any> =>
        (p as any).catch((e: any) => { console.warn('getLegalAnalysisStats partial failure:', e?.message); return [{}]; });
      const [flightStats, enterpriseStats, shellStats, watchtowerStats, biometricStats, josiahStats, ecgStats, chainStats] = await Promise.all([
        safe(sql`SELECT
          ...
          FROM (
            SELECT registration, taxonomy_tag, callsign, altitude, detection_timestamp
            FROM live_flight_detections_rows
            WHERE detection_timestamp > NOW() - INTERVAL '7 days'
            LIMIT 200000
          ) s`),
        safe(sql`SELECT COUNT(DISTINCT entity_name)::int as enterprise_count FROM criminal_enterprise_command_structure`),
        safe(sql`SELECT (SELECT COALESCE(reltuples,-1)::bigint FROM pg_class WHERE relname='shell_companies') as total`),
        safe(sql`SELECT (SELECT COALESCE(reltuples,-1)::bigint FROM pg_class WHERE relname='watchtower_unified_master') as total`),
        safe(sql`SELECT (SELECT COALESCE(reltuples,-1)::bigint FROM pg_class WHERE relname='biometric_monitoring') as total, ROUND(AVG(NULLIF(heart_rate,0))::numeric,0)::int as avg_hr FROM (SELECT heart_rate FROM biometric_monitoring WHERE COALESCE(measurement_timestamp,created_at) > NOW() - INTERVAL '30 days' LIMIT 200000) b`),
        safe(sql`SELECT (SELECT COALESCE(reltuples,-1)::bigint FROM pg_class WHERE relname='josiah_reflections_rows') as total`),
        safe(sql`SELECT (SELECT COALESCE(reltuples,-1)::bigint FROM pg_class WHERE relname='physician_verified_ecgs') as total`),
        safe(sql`SELECT (SELECT COALESCE(reltuples,-1)::bigint FROM pg_class WHERE relname='evidence_chain_links') as total`),
      ]);
    ...
    case 'getFederalCaseConvergence': {
      const safeC = <T,>(p: Promise<T>): Promise<any> =>
        (p as any).catch((e: any) => { console.warn('getFederalCaseConvergence partial failure:', e?.message); return [{}]; });
      const [flightSt, biometricSt, ecgSt, josiahSt, ocrSt, convergenceCalc] = await Promise.all([
        safeC(sql`SELECT (SELECT COALESCE(reltuples,-1)::bigint FROM pg_class WHERE relname='live_flight_detections_rows') as total_flights, COUNT(DISTINCT registration) as unique_aircraft, COUNT(CASE WHEN taxonomy_tag IN ('xxb_kcso','xxb_tier1_priority','xxb_kcso_shell') THEN 1 END) as priority_hits FROM (SELECT registration, taxonomy_tag FROM live_flight_detections_rows WHERE detection_timestamp > NOW() - INTERVAL '7 days' LIMIT 200000) s`),
        safeC(sql`SELECT (SELECT COALESCE(reltuples,-1)::bigint FROM pg_class WHERE relname='biometric_monitoring') as total, ROUND(COALESCE(AVG(NULLIF(heart_rate,0)),0)::numeric,0) as avg_hr FROM (SELECT heart_rate FROM biometric_monitoring WHERE COALESCE(measurement_timestamp,created_at) > NOW() - INTERVAL '30 days' LIMIT 200000) b`),
        safeC(sql`SELECT (SELECT COALESCE(reltuples,-1)::bigint FROM pg_class WHERE relname='physician_verified_ecgs') as total`),
        safeC(sql`SELECT (SELECT COALESCE(reltuples,-1)::bigint FROM pg_class WHERE relname='josiah_reflections_rows') as total`),
        safeC(sql`SELECT (SELECT COALESCE(reltuples,-1)::bigint FROM pg_class WHERE relname='ocr_aircraft_holding_patterns') as total`),
        safeC(sql`WITH daily_factors AS (SELECT DATE(detection_timestamp) as event_date, COUNT(*) as flight_count FROM (SELECT detection_timestamp, taxonomy_tag FROM live_flight_detections_rows WHERE detection_timestamp > NOW() - INTERVAL '30 days' LIMIT 300000) d WHERE taxonomy_tag IN ('xxb_kcso','xxb_tier1_priority','xxb_kcso_shell','xxb_tier2_shell') GROUP BY 1), biometric_days AS (SELECT DATE(COALESCE(measurement_timestamp,created_at)) as event_date, COUNT(*) as bio_count, COALESCE(AVG(NULLIF(heart_rate,0)),0) as avg_hr FROM biometric_monitoring WHERE COALESCE(heart_rate,0)>90 AND COALESCE(measurement_timestamp,created_at) > NOW() - INTERVAL '30 days' GROUP BY 1), convergence AS (SELECT f.event_date, f.flight_count, COALESCE(b.bio_count,0) as bio_count, COALESCE(b.avg_hr,0) as avg_hr FROM daily_factors f LEFT JOIN biometric_days b ON f.event_date=b.event_date) SELECT COUNT(*) as total_convergence_days, COUNT(CASE WHEN flight_count>0 AND bio_count>0 THEN 1 END) as two_factor_events, SUM(flight_count) as total_flights_in_convergence, ROUND(AVG(avg_hr)::numeric,0) as avg_hr_in_events FROM convergence`),
      ]);
